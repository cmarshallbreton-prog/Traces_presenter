...
Cette version regroupe les traitements de trois corpus de traces :
...
`pipeline_utils.py` lance chaque indicateur dans un thread séparé. Cela évite les collisions de variables globales, de loggers, d’imports et de `sys.argv` entre scripts historiques.
...
\[
SessionSpanMinutes = \frac{\max(t)-\min(t)}{60}
\]

où `t` désigne les timestamps des `Run.Test`, en secondes.

Il s’agit d’une **étendue calendaire**, pas d’un temps de travail actif : toutes les pauses intermédiaires sont incluses.
...
**Champs :** `actor`, `timestamp.$date`, `filename_infere`, `session.id`, `P_codeState`, `tests[].verdict`.

Les erreurs sont identifiées à une granularité **grossière** : `FailedVerdict` et `ExceptionVerdict`.

Les `Run.Test` sont triés, segmentés par fichier/session lorsque les champs sont disponibles, et les relances consécutives à code inchangé sont ignorées.
...
**Unité :** `(session.id, fichier, fonction)`.
...
**Unité de transition :** `(session.id, fichier)`.
...
**Code :** `P_codeState`  
**Transitions :** consécutives au sein du même `(session.id, fichier)`.
...
Pour chaque paire consécutive de `Run.Test` ayant deux `P_codeState` exploitables, dans le même `(session.id, fichier)` :
...
Le code historique fixe `GAP_TIME = 1200` et compare cette valeur à un écart exprimé en **minutes** dans `assign_session_ids`. Cette convention paraît inhabituelle mais n’a pas été corrigée, afin de ne pas modifier indirectement EQ, RED ou WatWin. Dans le fichier `MainTable.csv` fourni, `SessionID` n’est pas présent : ce comportement historique est donc effectivement utilisé.
...
# 10. Points conservés volontairement pour la reproductibilité

Certaines conventions historiques de ProgSnap2 sont atypiques mais n’ont pas été « corrigées » silencieusement :

- `eq.py`, `red.py`, `watwin.py` sont inchangés ;
- leurs dépendances historiques conservent leur logique ;
- `SessionCount` ProgSnap2 garde sa sémantique de moyenne de sous-sessions ;
- le traitement de `GAP_TIME = 1200` reste celui du code historique ;
- `MinutesToScore1` conserve la pénalité fixe de 60 minutes quand le score 1 n’est jamais atteint.

Ces choix sont documentés ici afin qu’une modification méthodologique future puisse être faite **explicitement**, avec un nouveau nom/version d’indicateur plutôt qu’en changeant rétroactivement la définition des données déjà analysées.
