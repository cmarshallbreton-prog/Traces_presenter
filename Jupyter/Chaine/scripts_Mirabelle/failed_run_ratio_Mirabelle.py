...
def calculate_failed_run_ratio(actor_rows) -> float | None:
    """
    Calcule le ratio de Run.Test non vides contenant au moins un FailedVerdict
    ...
    return runs_with_failed / total_non_empty_runs
...
def main(read_path: str, write_path: str) -> None:
    ...
    for actor in sorted(actors):
        ...
        ratio = calculate_failed_run_ratio(actor_rows)
...
if __name__ == "__main__":
    ...
    _write = sys.argv[2] if len(sys.argv) > 2 else "out/FailedTestRunRatio.csv"
    main(_read, _write)
