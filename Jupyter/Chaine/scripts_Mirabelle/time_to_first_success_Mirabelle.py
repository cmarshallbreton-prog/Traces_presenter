"""Calcule le temps moyen jusqu'au premier succès dans Mirabelle.

L'unité est une fonction testée dans un fichier et une session. Pour chaque
unité qui atteint un succès, le temps est la durée en minutes entre son premier
Run.Test observable et son premier Run.Test où tous ses cas de test passent.
La métrique étudiant est la moyenne de ces durées.

Les unités n'atteignant jamais un succès sont exclues (temps censuré).

Usage
-----
python time_to_first_success_Mirabelle.py [entree.csv] [sortie.csv]
"""
...
def calculate_metric(actor_rows: pd.DataFrame) -> tuple[float, int] | None:
    ...
    for _, unit_rows in actor_rows.groupby(group_cols, sort=False):
        successful_rows = unit_rows[unit_rows["__success"]]
        if successful_rows.empty:
            continue
        first_ts = unit_rows.iloc[0]["__timestamp"]
        success_ts = successful_rows.iloc[0]["__timestamp"]
        minutes = max((success_ts - first_ts).total_seconds() / 60.0, 0.0)
...
def main(read_path: str, write_path: str) -> None:
    ...
    for actor in sorted(actors):
        ...
        if result is None:
            out.warning("  %s : aucun temps jusqu'au succès mesurable — ignoré", actor)
            dropped += 1
            continue
        value, units = result
        metric_map[actor] = round(value, 6)
        out.info("  %s : %.3f min (%d unité(s) réussie(s))", actor, value, units)
